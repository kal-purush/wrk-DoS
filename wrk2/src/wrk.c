...
int non_200_count = 0;
int *non_200_statuses;
int non_200_capacity = 100000000; // Initial capacity

void init_non_200_statuses() {
    non_200_statuses = (int *)malloc(non_200_capacity * sizeof(int));
    if (non_200_statuses == NULL) {
        fprintf(stderr, "Memory allocation failed\n");
        exit(EXIT_FAILURE);
    }
}

void add_non_200_status(int status) {
    if (non_200_count >= non_200_capacity) {
        non_200_capacity *= 2;
        non_200_statuses = (int *)realloc(non_200_statuses, non_200_capacity * sizeof(int));
        if (non_200_statuses == NULL) {
            fprintf(stderr, "Memory allocation failed\n");
            exit(EXIT_FAILURE);
        }
    }
    non_200_statuses[non_200_count++] = status;
}



struct latency_stuc latency_arr[3000000];
long long t_latency=0;
char *latency_str = "";
char *req_s_str = "";
int time_count=-1;
...
char *framework = "";
char *payload = "";
char *concurrency_number="";
char *connection_number="";
char *file_number_extension="";
char *request_limit="";
char *parent_file_path="";
...
static void usage() {
    printf("Usage: wrk <options> <url>                            \n"
           ...
           "    -m <malicious>         Malicious flag (true/false)\n"
           "    -p <file_path>         Path to the file           \n"
...
void *calculateReqpers(void *vargp)
{   
    int initial_count=0;
    while (true)
    {   
        time_t current_time;
        time(&current_time);
        int req_this_sec = count-initial_count;
        initial_count = count;
        char temp[1000]  = "";
        // snprintf(temp, sizeof(temp), "%d %s\n", req_this_sec, ctime(&current_time));
        snprintf(temp, sizeof(temp), "%d %d %s\n", req_this_sec,time_count, ctime(&current_time));
        char* p = &temp[0];
        req_s_str = append_strings(req_s_str, p);
        time_count+=1;
        sleep(1);
    }
    return NULL;
...
// void get_framwork_and_payload(int argc, char **argv){
//     for (int i = 0; i < argc; i++){
//         // printf("%d -> %s\n",i,argv[i]);
//         if(i==argc-8){
//             concurrency_number=argv[i];
//         }
//         if(i==argc-7){
//             connection_number=argv[i];
//         }
//         if(i==argc-6){
//             framework=argv[i];
//         }
...
//         }
//         if(i==argc-4){
//             int maliciousIndicator=atoi(argv[i]);
//             isMalicious = maliciousIndicator==1?true:false;
//         }
//         if(i==argc-3){
//             file_number_extension = argv[i];
//         }

//         if(i==argc-2){
//             request_limit = argv[i];
//         }
//         if(i==argc-1){
//             parent_file_path = argv[i];
//         }
...
int main(int argc, char **argv) {
    // get_framwork_and_payload(argc, argv);
    init_non_200_statuses();
    pthread_t thread_id;
    pthread_create(&thread_id, NULL, calculateReqpers, NULL);
...
//    if (cfg.latency) print_stats_latency(latency_stats);
    ...
    clock_t begin = clock();
    // printf("  Starting array processing\n");
        ...
        // strcat(filename1,parent_file_path);
        // strcat(filename1,framework);
        // filename1[strlen(filename1)]='/';
        // char* fname = "latency_per_req_wrk_";
        // strcat(filename1,fname);
        // strcat(filename1,payload);
        // filename1[strlen(filename1)]='_';
        // strcat(filename1,concurrency_number);
        // filename1[strlen(filename1)]='_';
        // strcat(filename1,connection_number);
        // filename1[strlen(filename1)]='_';
        // strcat(filename1,request_limit);
        // filename1[strlen(filename1)]='_';
        // strcat(filename1,file_number_extension);
        // char* ext = ".txt";
        ...
        // {
        //     printf("Error!");
        //     fprintf(stderr, "%s", filename1);   
        //     exit(1);             
        // }
    char latency_file_path[1000];
    // snprintf(throughput_file_path, sizeof(throughput_file_path), "%s/req_per_sec_benign.txt", file_path);
    snprintf(latency_file_path, sizeof(latency_file_path), "%s/latency_per_req_%s.txt", file_path, file_ext);
    FILE *fptr = fopen(latency_file_path,"w");
    if(fptr == NULL)
    {
        printf("Error!");
        fprintf(stderr, "%s", latency_file_path);   
        exit(1);             
    }
    for(int i=0;i<count;i++){
        fprintf(fptr,"%llu %d %ld\n",latency_arr[i].latency, latency_arr[i].time_count, latency_arr[i].current_time); 
    }
    fclose(fptr);

    char error_file_path[1000];
    // snprintf(throughput_file_path, sizeof(throughput_file_path), "%s/req_per_sec_benign.txt", file_path);
    snprintf(error_file_path, sizeof(error_file_path), "%s/error_stat_%s.txt", file_path, file_ext);
    ...
    for (int i = 0; i < non_200_count; i++) {
        fprintf(file, "%d\n", non_200_statuses[i]);
    }
    fclose(file);

    char throughput_file_path[1000];
    // snprintf(throughput_file_path, sizeof(throughput_file_path), "%s/req_per_sec_benign.txt", file_path);
    snprintf(throughput_file_path, sizeof(throughput_file_path), "%s/req_per_sec_%s.txt", file_path, file_ext);

    fileWrite(throughput_file_path, req_s_str);

    clock_t end = clock();
    double time_spent = (double)(end - begin) / CLOCKS_PER_SEC;
    printf("  Done array processing %f\n", time_spent);
...
static int response_complete(http_parser *parser) {
    ...
    if (status == 200){
        ...
        // time_t current_time;
        // time(&current_time);
        latency_arr[count].time_count = time_count;
        latency_arr[count].current_time = now;
        latency_arr[count].latency = expected_latency_timing;
    }else{
        add_non_200_status(status);
    }
...
static struct option longopts[] = {
    ...
    { "malicious_flag", required_argument, NULL, 'm' },
...
static int parse_args(struct config *cfg, char **url, struct http_parser_url *parts, char **headers, int argc, char **argv) {
    ...
    while ((c = getopt_long(argc, argv, "t:c:d:s:m:p:H:T:R:LUBrv?", longopts, NULL)) != -1) {
        switch (c) {
            case 't':
            ...
            case ':':
            case 'm':
                malicious_flag = optarg;
                printf("Malicious Flag: %s\n", malicious_flag);
                if (strcmp(malicious_flag, "1") == 0) {
                    file_ext = "malicious";
                } else {
                    file_ext = "benign";
                }
